...
fn main() {
    ...
    match input {
        Ok(mut guard_events) => {
            ...
            for (k, v) in guard_sleep_schedule.iter() {
                for (k2, v2) in v.iter() {
                    if v2 > &best_min_count {
                        best_min_count = v2.clone();
                        best_min = k2.minute();
                        guard_id = k.clone();
                    }
                }
            ...
            println!("Best minute is {:?} with {:?} total sleeps found by guard {:?}", best_min, best_min_count, guard_id);
